...
const ColdBrew = () => {
    ...
    useEffect(() => {
        const coldBrewData = data.filter(milkCoffes => milkCoffes.category === 'ColdBrew').slice(0, 6)
        setColdBrew(coldBrewData)
    }, [data])
    ...
    return (
        <div className="w-11/12 mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {
                    coldBrew.map(item => <div onClick={() => navigate(`/${item.id}`)} key={item.id} className="card bg-base-100 gap-5 lg:w-[350px] shadow-xl hover:scale-105 border cursor-pointer">
